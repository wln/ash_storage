...
- ~~**Layers and encryption**~~ ✅ — Ordered logical IO layers (`AshStorage.Layer`) for byte transforms, serving policy, and durable per-blob metadata, with fail-closed reads when a persisted layer isn't configured at runtime. Includes bundled AES-256-GCM envelope encryption (`AshStorage.Layer.Encryption`) behind a pluggable key-manager behaviour (a Cloak key manager ships built in) with DEK rewrap. See [Layers](documentation/topics/layers.md) and [Encryption](documentation/topics/encryption.md); the lower-level IO boundary is documented on the `AshStorage.BlobIO` module.
...
- ~~**Checksum verification (partial)**~~ ✅ — Server-side uploads send `Content-MD5` so S3/Azure reject corrupted bodies at the edge; Azure also persists the MD5 via `x-ms-blob-content-md5`. Direct uploads are auto-confirmed by `AttachBlob` against `Service.head/2` before linking. Downloads are verified through the BlobIO read path. Multipart/block-based verification is documented in `documentation/topics/checksum-verification.md` and ships when multipart upload itself does.
...
- **Checksum verification follow-ups** — `Put Blob` sends both `Content-MD5` and `x-ms-blob-content-md5`, and downloads verify against the stored blob checksum. Block upload verification remains.
